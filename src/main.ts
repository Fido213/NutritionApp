...
import { createBootOverlay } from './ui/components/loading-card';
...
async function initApp() {
  ...
  try {
    ...
    // Curated aliases (rice family et al.): idempotent, never overwrites
    // user pins, converges already-seeded installs with no wipe.
    try {
      const { applyCuratedAliases } = await import('@services/food/curated-aliases');
      const aliasRes = await applyCuratedAliases(ctx.foodRepo, ctx.aliasRepo);
      if (aliasRes.applied > 0) console.log('[aliases] curated applied:', aliasRes.applied);
    } catch (e) { console.debug('[aliases] skipped', e); }

    // Interpreter warm, AWAITED (was fire-and-forget): the names-only fetch
    // + index build + embedding prewarm must finish behind the boot card,
    // never under the user's first tap. Failure still boots (submit refetches).
    ...
    try {
      ...
      try {
        const { interpretText } = await import('@services/interpreter');
        await interpretText('warmup probe', foods.length ? foods : null);
      } catch { /* warm best-effort only */ }
